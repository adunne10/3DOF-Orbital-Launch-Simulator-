...
lat_launch=deg2rad(28.7); %cape canaveral
...
sol_0=   [10000 ; 10  ; deg2rad(3) ; 10]; %initial guess for m_pld , t_coast , theta_kick , t_pitchover & t_pitch
lowbound=[ 100 ; 2 ; deg2rad(1) ; 5 ]; %best guess for lower bounds
upbound= [ 20000 ; 20 ; deg2rad(10) ; 20 ]; %best guess for upper bounds

objective=@(sol) -sol(1)/1e3;
nonlcon=@(sol) constraints(sol,stage,g,h_orbit,v_orbit,theta_SECO_goal,v_earth,azimuth,t_pitch);

options=optimoptions('fmincon', 'Algorithm', 'interior-point', 'MaxFunctionEvaluations',1000 ,'Display','iter','StepTolerance',1e-8,'ConstraintTolerance',1e-4, 'MaxIterations', 1000);
...
function [c,ceq] = constraints(sol,stage,g,h_target,v_target,theta_SECO_goal,v_earth,azimuth,t_pitch)
...
T=abs(stage(1).thrust-(P_atm*(stage(1).A_exit)));
...
[t2,Y2]=ode45(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,g,t_ignition_2,t_burn_2,theta_SES,dthetadt_SES),[t_ignition_2 t_SECO],Y0_2);
...
k = (tan(theta_SECO_goal) - a) / b;
theta_2 = atan(a + b * k * tau_2);
...
theta_SECO=theta_2(end);
    ...
    function dY2dt= burn_2(t,Y2,stage,m_dot_2,g,t_ignition_2 ,t_burn_2,theta_SES,dthetadt_SES)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
