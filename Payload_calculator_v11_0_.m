...
thetadeg=rad2deg(data.theta);
% theta_kick_deg=rad2deg(theta_kick);

t_MECO=stage(1).m_prop/(stage(1).thrust/(stage(1).isp*g));
t_SES=t_MECO+t_seperation;
t_SECO=t_SES+sol_optimal(5);
rho=zeros(size(data.h));
...
q=(0.5.*rho.*(data.v).^2)./1000; %units in kpa
max_q=max(q);
...
subplot(2,2,1);
plot(data.t,h_km) %altitude vs time
grid
hold on 
xline(t_MECO,'--r','MECO') 
xline(t_SES,'--r')
...
subplot(2,2,2);
plot(data.t,q) %dynamic pressure vs time
grid
hold on
xline(t_MECO,'--r','MECO') 
xline(t_SES,'--r')
...
subplot(2,2,3);
plot(h_km,h_km) %height vs downrange distance
grid
hold on
ylabel("altitude (km)")
...
subplot(2,2,4);
plot(data.t,thetadeg) % theta vs time
grid
hold on
xline(t_MECO,'--r','MECO') 
xline(t_SES,'--r')
...
fprintf('e = %.2f \n', e)
fprintf('m_pld_max = %.2f kg\n', (sol_optimal(1)+(m_prop_remaining-m_prop_2b)))
...
[t2,Y2]=ode45(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,g,t_ignition_2,t_burn_2,theta_SES,theta_SECO,dthetadt_SES),[t_ignition_2 t_SECO_a],Y0_2,opts_ode);
...
function [h_apo, h_per, e, m_prop_2b, m_prop_remaining] = orbit_calc(data, h_orbit,sol)
...
function dY2dt= burn_2(t,Y2,stage,m_dot_2,g,t_ignition_2,t_burn_2,theta_SES,theta_SECO,dthetadt_SES)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
