...
sol_0=[-0.05;0;50;5;0.2;30]; %initial guess for alpha,beta,m_pld & t_coast

lowbound=[-5;-5;100;1;0.1;5]; %best guess for lower bounds
upbound=[0;5;30000;15;10;60]; %best guess for upper bounds

objective=@(sol) -sol(3)/1e3;

nonlcon=@(sol) constraints(sol,stage,R,g,h_orbit,v_orbit,m_earth,r_earth);
options=optimoptions('fmincon', 'Algorithm', 'interior-point', 'EnableFeasibilityMode',true, 'Display','iter','StepTolerance',1e-8,'ConstraintTolerance',1e-4, 'OptimalityTolerance', 1e-4, 'MaxIterations', 200);
...
t_coast_optimal=sol_optimal(4)
gain_optimal=sol_optimal(5)
t_pitchover_optimal=sol_optimal(6)
...
[h_SECO, v_SECO, theta_SECO] = trajectory(alpha, beta, m_pld,stage,R,g,t_coast,gain,t_pitchover);

    % equality constraints: hit target altitude, velocity, horizontal theta=0
    ceq = [ (h_SECO - h_target)/1000;
            (v_SECO - v_target)/100];
    c = abs(theta_SECO-0)-deg2rad(5); % no inequality constraints
end 

function [h_SECO, v_SECO, theta_SECO] = trajectory(alpha, beta ,m_pld,stage,R,g,t_coast,gain,t_pitchover)
...
Y0_1=[0;0;m0;0;pi/2]; %h=0, v=0, m=m0, x=0, theta=pi/2 (vertical)

[t1,Y1]=ode45(@(t,Y1) burn_1(t,Y1,stage,m_dot_1,R,g,alpha,beta,gain,t_pitchover),[0 t_burn_1],Y0_1);
...
function dY1dt= burn_1(t,Y1,stage,m_dot_1,R,g,alpha,beta,gain,t_pitchover)
h=Y1(1);
v=Y1(2);
m=Y1(3);
x=Y1(4);
...
theta_command=atan(alpha*(t-t_pitchover)+beta)+pi/2;
else
    theta_command=pi/2;
end
...
dmdt=m_dot_1;
dthetadt=(theta_command-theta)*gain;
...
[t_sep,Y_sep]=ode45(@(t,Y_sep) seperation(t,Y_sep,stage,R,g,alpha,beta,gain,t_pitchover),[t_burn_1 t_ignition_2],Y0_sep);
...
function dY_sepdt= seperation(t,Y_sep,stage,R,g,alpha,beta,gain,t_pitchover)
h=Y_sep(1);
v=Y_sep(2);
m=Y_sep(3);
x=Y_sep(4);
...
[t2,Y2]=ode45(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,R,g,alpha,beta,gain,t_pitchover),[t_ignition_2 t_SECO],Y0_2);
...
function dY2dt= burn_2(t,Y2,stage,m_dot_2,R,g,alpha,beta,gain,t_pitchover)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
...
% 
% figure%3
% plot(theta_1_deg,h_1_km) % theta vs altitude
% grid
% hold on
% plot (theta_sep_deg,h_sep_km)
% plot (theta_2_deg,h_2_km)
