...
vehicle_payload_data=readtable("Kourou_5deg_400km_rtls_alu.csv");
...
FU_stage_N_MYr = nan(nCfg,nN);
FU_engine_N_MYr = nan(nCfg,nN);
ground_ops_ex_MYr =nan(nCfg,nN);
MC_MYr=nan(nCfg,nN);
recovery_MYr=nan(nCfg,nN);
...
recovery_1=(1.5/5).*(7*5.^0.7+(m_dry_1).^0.83).*f_8;
recovery_2=(1.5/5).*(7*5.^0.7+(m_dry_2).^0.83).*f_8;

f_v=1.0;

ground_ops_N=8*(m_wet.^0.67).*(5.^(-0.9))*(2^0.7)*0.5*f_v*f4_stage*f_8;
MC_N=20*1.4*5^(-0.65)*f4_stage*f_8;
...
DD_total_re_MYr=(f_0.*(DD_stage_1_re+DD_stage_2_ex+DD_engine_1+DD_engine_2).*f_6.*f_7.*f_8); 

FU_stage_N=f_0^2.*(FU_stage_1+FU_stage_2).*f_8; %wrkyr
FU_engine_N=f_0^2.*(FU_engine_1+FU_engine_2).*f_8; %wrkyr

FU_stage_N_MYr(:,k)=FU_stage_N;
FU_engine_N_MYr(:,k)=FU_engine_N;
ground_ops_ex_MYr(:,k)=ground_ops_N;
MC_MYr(:,k)=MC_N;
recovery_MYr(:,k)=recovery_1;


end

FU_stage_ave=mean(FU_stage_N_MYr(:,1:5),2); 
FU_engine_ave=mean(FU_engine_N_MYr(:,1:5),2); 

FU_engine_refurb=FU_engine_ave*0.1;
FU_stage_refurb=FU_stage_ave*0.02;

FU_ave_re=(FU_stage_ave+FU_engine_ave+9*FU_engine_refurb+9*FU_stage_refurb)/10; %assumes 10 flights per booster
GOps_ave_re=mean(ground_ops_ex_MYr,2); %MYr
MC_ave_re=mean(MC_MYr,2); %MYr
recovery_ave_re=mean(recovery_MYr,2); %MYr
...
vehicle_payload_cost_data = table(ID,F1,EC1,T1,N1,T_stage1,m_dry_1,m_prop_1,isp_1, ...
                ...
                m_wet,max_payload,GLOW,DD_total_re_MYr,C_prop, ...
                FU_ave_re,GOps_ave_re,MC_ave_re,recovery_ave_re);
...
outFile = "COSTED_Kourou_5deg_400km_rtls.csv";
