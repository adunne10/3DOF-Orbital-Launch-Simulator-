...
obj_ps = @(sol) penaltyObjective(sol, sol_scale, h_orbit, objective);
...
sol__optimal_scaled = patternsearch(obj_ps, sol_0, [],[],[],[], lowbound, upbound, [], opts);
...
opts_ode = odeset('RelTol',1e-6,'AbsTol',1e-8,'MaxStep',2.0);
...
[t1a,Y1a]=ode113(@(t,Y1a) burn_1a(t,Y1a,stage,m_dot_1,g,t_pitchover,theta_kick,t_pitch),[0 t_burn_1a],Y0_1a,opts_ode);
...
[t1b,Y1b]=ode113(@(t,Y1b) burn_1b(t,Y1b,stage,m_dot_1,g),[t_burn_1a t_burn_1b+t_burn_1a],Y0_1b,opts_ode);
...
[t_sep,Y_sep]=ode113(@(t,Y_sep) seperation(t,Y_sep,stage,g),[t_burn_1 t_ignition_2],Y0_sep,opts_ode);
...
[t2,Y2]=ode113(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,g,t_ignition_2,t_burn_2,theta_SES,theta_SECO),[t_ignition_2 t_SECO_a],Y0_2,opts_ode);
...
dY2dt = [dhdt;dvdt;dmdt;dxdt];
    ...
    function J = penaltyObjective(sol, sol_scale, h_orbit, objective)
    [c,ceq] = constraints(sol, sol_scale, h_orbit);
    rhoIneq = 1e6;   % tune
    rhoEq   = 1e8;   % usually larger
    J = objective(sol) ...
        + rhoIneq*sum(max(c,0).^2) ...
        + rhoEq*sum(ceq.^2);
end