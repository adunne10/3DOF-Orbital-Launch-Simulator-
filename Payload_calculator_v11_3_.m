...
lat_launch=deg2rad(40); %cape canaveral
stage=vehicle.stage;
...
exitflag_vec  = nan(N,1);       % fmincon exit flag
...
h_SECO=data.h(end);
    ...
    c = [(m_prop_2b-m_prop_remaining)/1000;
         (h_min-h_per)/10000;
         (h_cut_min - h_SECO)/10000
    ];
end 
...
rhs1a = @(t,Y) burn_1a(t,Y,stage,m_dot_1,g,t_pitchover,theta_kick,t_pitch);

% Minimum mass threshold during stage 1: (stage1 dry + stage2 dry+prop + payload)
m_min_1 = stage(1).m_dry + stage(2).m_dry + stage(2).m_prop + m_pld;

[t1a, Y1a] = rk4_integrate(rhs1a, 0, t_burn_1a, Y0_1a, dt1, ...
    @(t,Y) stop_ascent(t,Y,m_min_1));
...
rhs1b = @(t,Y) burn_1b(t,Y,stage,m_dot_1,g);

[t1b, Y1b] = rk4_integrate(rhs1b, t_burn_1a, t_burn_1, Y0_1b, dt1, ...
    @(t,Y) stop_ascent(t,Y,m_min_1));
...
rhs_sep = @(t,Y) seperation(t,Y,stage,g);

% During coast after dropping stage1 dry in your code, min is stage2 dry+prop+payload
m_min_sep = stage(2).m_dry + stage(2).m_prop + m_pld;

[t_sep, Y_sep] = rk4_integrate(rhs_sep, t_burn_1, t_ignition_2, Y0_sep, dtsep, ...
    @(t,Y) stop_ascent(t,Y,m_min_sep));
...
rhs2 = @(t,Y) burn_2(t,Y,stage,m_dot_2,g,t_ignition_2,t_burn_2,theta_SES,theta_SECO);

t_SECO_a = t_ignition_2 + t_burn_2;

% Stage2 minimum mass = stage2 dry + payload
m_min_2 = stage(2).m_dry + m_pld;

[t2, Y2] = rk4_integrate(rhs2, t_ignition_2, t_SECO_a, Y0_2, dt2, ...
    @(t,Y) stop_ascent(t,Y,m_min_2));

tau = (t2 - t_ignition_2)./t_burn_2;
s = 3.*tau.^2 - 2.*tau.^3;     % smoothstep
theta_2 = theta_SES + (theta_SECO - theta_SES).*s;

t1b = t1b(2:end); Y1b = Y1b(2:end,:);
t_sep = t_sep(2:end); Y_sep = Y_sep(2:end,:);
t2 = t2(2:end); Y2 = Y2(2:end,:);
...
data.theta=[Y1a(:,5) ;Y1b(:,5) ;Y_sep(:,5) ;theta_2];
...
function [t_hist, Y_hist] = rk4_integrate(rhs, t0, tf, Y0, dt, stopfun)
%RK4_INTEGRATE Fixed-step RK4 integration from t0 to tf.
%   rhs(t,Y) must return dY/dt as a column vector (same size as Y).
%   Y0 can be row or column; it will be treated as a column internally.
%   stopfun(t,Y) -> true to stop early (optional).

    if nargin < 6 || isempty(stopfun)
        stopfun = @(t,Y) false;
    end

    if dt <= 0
        error("rk4_integrate: dt must be positive.");
    end

    Y = Y0(:);   % force column
    t = t0;

    % If no time to integrate, return the initial point only.
    if tf <= t0
        t_hist = t0;
        Y_hist = Y.';
        return
    ...
    % Number of steps (ceil so we reach/overshoot and then clamp last step)
    N = ceil((tf - t0)/dt);

    t_hist = zeros(N+1, 1);
    Y_hist = zeros(N+1, numel(Y));
    ...
    for i = 1:N
        ...
        % If we are at tf (or extremely close), stop.
        if dt_i <= 0
            t_hist = t_hist(1:i);
            Y_hist = Y_hist(1:i,:);
            return
        ...
        % Take one RK4 step
        Y = rk4_step(rhs, t, Y, dt_i);
        t = t + dt_i;

        t_hist(i+1)   = t;
        Y_hist(i+1,:) = Y.';

        % Early stop on user condition or non-finite state
        if any(~isfinite(Y)) || stopfun(t, Y)
            t_hist = t_hist(1:i+1);
            Y_hist = Y_hist(1:i+1,:);
            return
        end
...
function Ynext = rk4_step(rhs, t, Y, dt)
    k1 = rhs(t,        Y);
...
function stop = stop_ascent(~, Y, m_min)
h = Y(1);
m = Y(3);
stop = (h < 0) || (m < m_min);
end
...
function dY2dt= burn_2(t,Y2,stage,m_dot_2,g,t_ignition_2,t_burn_2,theta_SES,theta_SECO)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
