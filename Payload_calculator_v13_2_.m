...
h_min = 120e3;
c_per = (h_min - h_per)/1000;
per_marg = (h_per - h_min)/1000;

fprintf('h_per=%.3f km | per_marg=%.3f km | c_per=%.6f | exitflag=%d | constrviol=%.6g\n', ...
    h_per/1000, per_marg, c_per, exitflag, output.constrviolation);
...
function animate_trajectory(data)
    % Animate rocket trajectory in an Earth-rotating (Earth-fixed) frame.
    % Earth is drawn as a circle and the rocket position is rotated by -omega*t.

    global r_earth omega_earth
    ...
    % Earth circle (fixed in Earth-fixed frame)
    ang = linspace(0, 2*pi, 600);
    xe  = r_earth*cos(ang);
    ze  = r_earth*sin(ang);
    ...
    plot(xe/1000, ze/1000, 'b', 'LineWidth', 1.5);
    xlabel('x_E (km)'); ylabel('z_E (km)');
    title('Launch + Pitch-over (Earth-rotating / Earth-fixed view)');

h_target = 500e3; % change to your target
rt = r_earth + h_target;
plot((rt*cos(ang))/1000, (rt*sin(ang))/1000, '--', 'LineWidth', 1.0);
legend('Earth','Target orbit','Trajectory','Rocket','Velocity','Location','bestoutside');


    % Handles
    traj   = plot(nan, nan, 'g-', 'LineWidth', 1.2);
    rocket = plot(nan, nan, 'ro', 'MarkerSize', 6, 'MarkerFaceColor', 'r');
    vArrow = quiver(0,0,0,0, 0, 'r', 'LineWidth', 1.2, 'MaxHeadSize', 2);
    txt    = text(0.02, 0.98, '', 'Units','normalized', 'VerticalAlignment','top');
    ...
    % Animation step (limit frames)
    N    = numel(t);
    step = max(1, round(N/60));
    ...
    for i = 1:step:N
        ...
        % Rotate velocity too (so arrow matches frame)
        vxE =  c*vx(i) + s*vz(i);
        vzE = -s*vx(i) + c*vz(i);

        % Rotate the whole history correctly (each point gets its own rotation)
        thHist = omega_earth * t(1:i);
        cH = cos(thHist);
        sH = sin(thHist);
        
        xEh =  cH.*x(1:i) + sH.*z(1:i);
        zEh = -sH.*x(1:i) + cH.*z(1:i);

        % Update plots
        set(traj,   'XData', xEh/1000, 'YData', zEh/1000);
        set(rocket, 'XData', xE/1000,  'YData', zE/1000);

        vscale = 1e-3; % tweak for arrow length
        set(vArrow, 'XData', xE/1000, 'YData', zE/1000, ...
                    'UData', vxE*vscale, 'VData', vzE*vscale);

        set(txt, 'String', sprintf('t = %.1f s\nh = %.1f km', t(i), h(i)/1000));
