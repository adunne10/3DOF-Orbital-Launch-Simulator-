...
%% constants
global g G m_earth r_earth mu h_orbit v_earth vehicle t_seperation t_pitch
...
sol_scale = [1000, 0.01, 10, 100, 0.1];
...
global g vehicle t_seperation t_pitch
...
[t2,Y2]=ode45(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,g,t_ignition_2,t_burn_2,theta_SECO),[t_ignition_2 t_SECO_a],Y0_2,opts_ode);
...
function dY2dt= burn_2(t,Y2,stage,m_dot_2,g,t_ignition_2,t_burn_2,theta_SECO)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
...
t_rel=t-t_ignition_2;
t_left = max(t_burn_2 - max(t_rel,0), 3);
dthetadt_req=(theta_SECO-theta)/t_left;
dtheta_tvc=dthetadt_req-dthetadt_grav;

arg=(m*(v+0.1)/T ) * dtheta_tvc;
if ~isfinite(arg)
    arg = 0;
end

arg=max(min(arg, 0.999), -0.999);
alpha=asin(arg);
