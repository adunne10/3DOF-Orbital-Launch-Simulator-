...
sol_0=[-0.5;0;500;5]; %initial guess for alpha,beta,m_pld & t_coast

lowbound=[-5;-5;100;1]; %best guess for lower bounds
upbound=[0;5;30000;15]; %best guess for upper bounds
...
[h_SECO, v_SECO, theta_SECO] = trajectory(alpha, beta, m_pld,stage,R,g,t_coast);
...
function [h_SECO, v_SECO, theta_SECO] = trajectory(alpha, beta ,m_pld,stage,R,g,t_coast)
...
Y0_1=[0;0;m0;0]; %h=0, v=0, m=m0, x=0

[t1,Y1]=ode45(@(t,Y1) burn_1(t,Y1,stage,m_dot_1,R,g,alpha,beta),[0 t_burn_1],Y0_1);

h_1=Y1(:,1);
v_1=Y1(:,2);
m_1=Y1(:,3);
x_1=Y1(:,4);
theta_1=atan(alpha.*t1+beta)+pi/2;
...
function dY1dt= burn_1(t,Y1,stage,m_dot_1,R,g,alpha,beta)
h=Y1(1);
v=Y1(2);
m=Y1(3);
x=Y1(4);
...
%t_coast=7; %varies from 5-10 seconds
t_ignition_2=t_burn_1+t_coast;


Y0_sep=[h_MECO;v_MECO;m_MECO;x_MECO]; %state at MECO 

[t_sep,Y_sep]=ode45(@(t,Y_sep) seperation(t,Y_sep,stage,R,g,alpha,beta),[t_burn_1 t_ignition_2],Y0_sep);

h_sep=Y_sep(:,1);
v_sep=Y_sep(:,2);
x_sep=Y_sep(:,4);
theta_sep=atan(t_sep.*alpha+beta)+pi/2;
...
function dY_sepdt= seperation(t,Y_sep,stage,R,g,alpha,beta)
h=Y_sep(1);
v=Y_sep(2);
m=Y_sep(3);
x=Y_sep(4);
...
[t2,Y2]=ode45(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,R,g,alpha,beta),[t_ignition_2 t_SECO],Y0_2);

h_2=Y2(:,1);
v_2=Y2(:,2);
m_2=Y2(:,3);
x_2=Y2(:,4);
theta_2=atan(alpha.*t2+beta)+pi/2;
...
function dY2dt= burn_2(t,Y2,stage,m_dot_2,R,g,alpha,beta)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
