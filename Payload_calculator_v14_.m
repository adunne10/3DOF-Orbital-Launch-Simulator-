...
objective = @(sol) obj_payload_leftover(sol, sol_scale, h_orbit, lambda,lat_launch,azimuth,isa);

nonlcon=@(sol) constraints(sol,sol_scale,h_orbit,lat_launch,azimuth,isa);
...
function [c,ceq] = constraints(sol,sol_scale,h_orbit,lat_launch,azimuth,isa)

global mu r_earth 

sol_unscaled=sol.*sol_scale';

data =sim(sol_unscaled,lat_launch,azimuth,isa);
...
[~,Y1a]=ode113(@(t,Y1a) burn_1a(t,Y1a,m_dot_1,t_pitchover,theta_kick,t_pitch,lat_launch,azimuth,isa),[0 t_burn_1a],Y0_1a,opts_ode);
...
[~,Y1b]=ode113(@(t,Y1b) burn_1b(t,Y1b,m_dot_1,lat_launch,azimuth,isa),[t_burn_1a t_burn_1b+t_burn_1a],Y0_1b,opts_ode);
...
function J = obj_payload_leftover(sol, sol_scale, h_orbit, lambda,lat_launch,azimuth,isa)
