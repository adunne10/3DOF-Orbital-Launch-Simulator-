...
[t1,Y1]=ode45(@(t,Y1) burn_1(t,Y1,stage,m_dot_1,R,g,theta_final,theta_0,t_SECO),[0 t_burn_1],Y0_1);

h_1=Y1(:,1);
v_1=Y1(:,2);
m_1=Y1(:,3);
x_1=Y1(:,4);
theta_1=pi/2-atan(tan(theta_0)-tan(theta_0-theta_final)*(t1/t_SECO));
...
function dY1dt= burn_1(t,Y1,stage,m_dot_1,R,g,theta_final,theta_0,t_SECO)
h=Y1(1);
v=Y1(2);
m=Y1(3);
x=Y1(4);
...
theta=pi/2-atan(tan(theta_0)-tan(theta_0-theta_final)*(t/t_SECO));
...
[t_sep,Y_sep]=ode45(@(t,Y_sep) seperation(t,Y_sep,stage,R,g,theta_0,theta_final,t_SECO),[t_burn_1 t_ignition_2],Y0_sep);

h_sep=Y_sep(:,1);
v_sep=Y_sep(:,2);
x_sep=Y_sep(:,4);
theta_sep=pi/2-atan(tan(theta_0)-tan(theta_0-theta_final)*(t_sep/t_SECO));
...
theta_SES=theta_sep(end);
    ...
    function dY_sepdt= seperation(t,Y_sep,stage,R,g,theta_0,theta_final,t_SECO)
h=Y_sep(1);
v=Y_sep(2);
m=Y_sep(3);
x=Y_sep(4);
...
[t2,Y2]=ode45(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,R,g,theta_0,theta_final,t_SECO),[t_ignition_2 t_SECO],Y0_2);

h_2=Y2(:,1);
v_2=Y2(:,2);
m_2=Y2(:,3);
x_2=Y2(:,4);
theta_2=pi/2-atan(tan(theta_0)-tan(theta_0-theta_final)*(t2/t_SECO));
...
function dY2dt= burn_2(t,Y2,stage,m_dot_2,R,g,theta_0,theta_final,t_SECO)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
...
figure%3
plot(t1,theta_1_deg) % theta vs altitude
grid
hold on
