...
sol_0=   [2 ; -2 ; -2 ; 15000 ; 10  ; deg2rad(1)]; %initial guess for alpha,beta,m_pld & t_coast

lowbound=[-5 ; -5 ; -5 ;10000 ; 5 ; deg2rad(0.1)]; %best guess for lower bounds
upbound= [5 ; 5 ; 5 ; 22000 ; 30 ; deg2rad(2)]; %best guess for upper bounds

objective=@(sol) -sol(4)/1e3;

nonlcon=@(sol) constraints(sol,stage,R,g,h_orbit,v_orbit,m_earth,r_earth,h_pitchover);
...
alpha_optimal=sol_optimal(1)
beta_optimal=sol_optimal(2)
charlie_optimal=sol_optimal(3)
m_pld_max=sol_optimal(4)
t_coast_optimal=sol_optimal(5)
theta_kick_optimal=sol_optimal(6)

function [c,ceq] = constraints(sol,stage,R,g,h_target,v_target,~,~,h_pitchover)
alpha=sol(1);
beta=sol(2);
charlie=sol(3);
m_pld=sol(4);
...
[h_SECO, v_SECO, theta_SECO] = trajectory(alpha, beta, charlie,m_pld,stage,R,g,t_coast,h_target,h_pitchover, theta_kick);

    % equality constraints: hit target altitude, velocity, horizontal theta=0
    ceq =  (h_SECO - h_target)/1000;
           
    c = [
    abs((v_SECO - v_target)/100 -1)
    abs((theta_SECO-0)-deg2rad(1)); % inequality constraints
    ];
end 

function [h_SECO, v_SECO, theta_SECO] = trajectory(alpha, beta , charlie, m_pld,stage,R,g,t_coast,h_orbit,h_pitchover, theta_kick)
...
[t2,Y2]=ode45(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,R,g,alpha,beta,charlie,h_orbit,theta_SES,h_SES),[t_ignition_2 t_SECO],Y0_2);

h_2=Y2(:,1);
v_2=Y2(:,2);
m_2=Y2(:,3);
x_2=Y2(:,4);
theta_2=theta_SES +alpha.*((h_2-h_SES)./h_orbit)+beta.*((h_2-h_SES)./h_orbit).^2+charlie.*((h_2-h_SES)./h_orbit).^3;
...
theta_SECO=theta_2(end);
    ...
    function dY2dt= burn_2(~,Y2,stage,m_dot_2,R,g,alpha,beta,charlie,h_orbit,theta_SES,h_SES)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
...
theta=theta_SES+alpha*((h-h_SES)/h_orbit)+beta*((h-h_SES)/h_orbit)^2+charlie*((h-h_SES)/h_orbit)^3;
...
[t2,Y2]=ode45(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,R,g,alpha_optimal,beta_optimal,charlie_optimal,h_orbit,theta_SES,h_SES),[t_ignition_2 t_SECO],Y0_2);

h_2=Y2(:,1);
v_2=Y2(:,2);
m_2=Y2(:,3);
x_2=Y2(:,4);
theta_2=theta_SES +alpha_optimal.*((h_2-h_SES)./h_orbit)+beta_optimal.*((h_2-h_SES)./h_orbit).^2+charlie_optimal.*((h_2-h_SES)./h_orbit).^3;
...
theta_SECO=theta_2(end);
    ...
    function dY2dt= burn_2(~,Y2,stage,m_dot_2,R,g,alpha_optimal,beta_optimal,charlie_optimal,h_orbit,theta_SES,h_SES)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
...
theta=theta_SES+alpha_optimal*((h-h_SES)/h_orbit)+beta_optimal*((h-h_SES)/h_orbit)^2+charlie_optimal*((h-h_SES)/h_orbit)^3;
