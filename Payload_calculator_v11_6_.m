...
[t2,Y2]=ode45(@(t,Y2) burn_2(t,Y2,stage,m_dot_2,g,theta_SES,theta_SECO,dthetadt_SES,t_burn_2,t_ignition_2),[t_ignition_2 t_SECO_a],Y0_2,opts_ode);

tau = (t2 - t_ignition_2)./t_burn_2;
tau = min(max(tau,0),1);

% In burn_2, after tau is computed:
dtheta0 = dthetadt_SES * t_burn_2;      % theta'(0) in "per tau" units
dtheta1 = 0;                             % theta'(1)
...
% Avoid divide-by-zero if theta1 ~ theta0
if abs(dTheta) < 1e-9
    theta_2 = theta0;
else
    k = dtheta0 / dTheta;                % normalized start slope

    % Clamp k to reduce overshoot (tune bounds if needed)
    k = min(max(k, -3), 3);

    % Hermite on s(tau): s(0)=0, s(1)=1, s'(0)=k, s'(1)=0
    s = (-2*tau.^3 + 3*tau.^2) + k*(tau.^3 - 2*tau.^2 + tau);

    theta_2 = theta0 + dTheta .* s;
end
...
function dY2dt= burn_2(t,Y2,stage,m_dot_2,g,theta_SES,theta_SECO,dthetadt_SES,t_burn_2,t_ignition_2)
h=Y2(1);
v=Y2(2);
m=Y2(3);
x=Y2(4);
