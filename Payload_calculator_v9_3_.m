...
burn_time_max=(stage(2).m_prop/(stage(2).thrust/(stage(2).isp*g)))-1;
%% Optimisation

sol_0=   [ 10000  ; 10 ; deg2rad(2)  ; 20 ; 7  ; 100 ; 2]; %initial guess for m_pld , t_coast , theta_kick , t_pitchover ,  t_pitch
lowbound=[ 100  ; 2  ; deg2rad(1)  ; 10 ; 5  ; 60 ; deg2rad(0)]; %best guess for lower bounds
upbound= [ 20000 ; 20 ; deg2rad(5)  ; 30 ; 10 ; burn_time_max;  deg2rad(10)]; %best guess for upper bounds
...
[h_SECO, v_SECO, m_SECO] = trajectory(m_pld,stage,g,mu,t_coast,t_pitchover, theta_kick,t_pitch,t_burn_2a, theta_SECO);
...
function [h_SECO,v_SECO,m_SECO] = trajectory( m_pld,stage,g,mu,t_coast,t_pitchover, theta_kick,t_pitch,t_burn_2a,theta_SECO)
...
b = [theta_SES; theta_SECO ; dthetadt_SES*t_burn_2a; 0];

theta_coeff = a\b;

Y0_2a=[h_SES;v_SES;m_SES;x_SES]; %state at SES

[t_2a,Y2a]=ode45(@(t,Y2a) burn_2(t,Y2a,stage,m_dot_2a,g,t_ignition_2a,t_burn_2a,theta_coeff,theta_SES,theta_SECO),[t_ignition_2a t_SECO],Y0_2a);
...
theta_2a = theta_coeff(1) + theta_coeff(2).*tau_2 + theta_coeff(3).*tau_2.^2 + theta_coeff(4).*tau_2.^3;
...
function dY2adt= burn_2(t,Y2a,stage,m_dot_2a,g,t_ignition_2a,t_burn_2a,theta_coeff,theta_SES,theta_SECO)
h=Y2a(1);
v=Y2a(2);
m=Y2a(3);
x=Y2a(4);
...
dthetadt = (theta_coeff(2) + 2*theta_coeff(3)*tau + 3*theta_coeff(4)*tau.^2)/t_burn_2a;

theta = min(theta, max(theta_SES,theta_SECO));
theta = max(theta, min(theta_SES,theta_SECO));
...
b = [theta_SES; theta_SECO_optimal; dthetadt_SES*t_burn_2a_optimal; 0];

theta_coeff = a\b;

Y0_2_a=[h_SES;v_SES;m_SES;x_SES]; %state at SES

[t_2a,Y2_a]=ode45(@(t,Y2_a) burn_2(t,Y2_a,stage,m_dot_2a,g,t_ignition_2a,t_burn_2a_optimal,theta_coeff,theta_SES,theta_SECO_optimal),[t_ignition_2a t_SECO_a],Y0_2_a);
...
function dY2adt= burn_2(t,Y2a,stage,m_dot_2a,g,t_ignition_2a,t_burn_2a_optimal,theta_coeff,theta_SES,theta_SECO_optimal)
h=Y2a(1);
v=Y2a(2);
m=Y2a(3);
x=Y2a(4);
...
% Evaluate cubic theta and derivative
theta = theta_coeff(1) + theta_coeff(2)*tau + theta_coeff(3)*tau.^2 + theta_coeff(4)*tau.^3;
dthetadt = (theta_coeff(2) + 2*theta_coeff(3)*tau + 3*theta_coeff(4)*tau.^2)/t_burn_2a_optimal;

theta = min(theta, max(theta_SES,theta_SECO_optimal));
theta = max(theta, min(theta_SES,theta_SECO_optimal));
