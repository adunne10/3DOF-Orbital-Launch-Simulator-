...
N = 10;                        %number of rows to simulate?
...
dV_stage2=dV_req*(stage.isp_2 / (stage.isp_1 + stage.isp_2));
exp_term=exp(dV_stage2/(stage.isp_2*const.g));
pld_guess=(stage.m_prop_2-stage.m_dry_2*(exp_term-1))/(exp_term-1);

pld_guess= max(pld_guess, 10);   % at least 10 kg
pld_guess = min(pld_guess, 0.15 * stage.m_wet);  % sanity upper bound
