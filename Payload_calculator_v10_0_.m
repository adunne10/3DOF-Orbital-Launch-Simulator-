...
sol_0=   [ pld_guess          ; deg2rad(2) ; 20 ; 7  ; burn_time_guess ; deg2rad(2)]; %initial guess for m_pld , t_seperation , theta_kick , t_pitchover ,  t_pitch
lowbound=[ stage(1).pld_guess ; deg2rad(1) ; 10 ; 5  ; 300             ; deg2rad(0)]; %best guess for lower bounds
upbound= [ stage(2).pld_guess ; deg2rad(5) ; 30 ; 10 ; burn_time_max   ; deg2rad(10)]; %best guess for upper bounds

objective=@(sol) -sol(1)/1e3;
nonlcon=@(sol) constraints(sol,stage,g,mu,h_orbit,r_earth,v_earth,t_seperation,dt);

options=optimoptions('fmincon', 'Algorithm','interior-point','FiniteDifferenceType','forward' ,'Display','iter','StepTolerance',1e-8,'ConstraintTolerance',1e-4, 'OptimalityTolerance', 1e-4, 'MaxIterations', 2000);
...
function [c,ceq] = constraints(sol,stage,g,mu,h_target,r_earth,v_earth,t_seperation,dt)
...
[h_SECO, v_SECO, m_SECO] = trajectory(m_pld,stage,g,t_pitchover, theta_kick,t_pitch,t_burn_2a, theta_SECO,t_seperation,dt);
...
function [h_SECO,v_SECO,m_SECO] = trajectory( m_pld,stage,g,t_pitchover, theta_kick,t_pitch,t_burn_2a,theta_SECO,t_seperation,dt)
...
[t1a, Y1a] = rk4_fixed(@(t,Y) burn_1a(t,Y,stage,m_dot_1,g,t_pitchover,theta_kick,t_pitch),0, t_burn_1a, Y0_1a, dt);
...
[t1b, Y1b] = rk4_fixed(@(t,Y) burn_1b(t,Y,stage,m_dot_1,g),t_burn_1a, t_burn_1, Y0_1b, dt);
...
[t_sep, Y_sep] = rk4_fixed(@(t,Y) seperation(t,Y,stage,g),t_burn_1, t_ignition_2a, Y0_sep, dt);
...
[t_2a, Y2a] = rk4_fixed(@(t,Y) burn_2(t,Y,stage,m_dot_2a,g,t_ignition_2a,t_burn_2a,theta_SES,theta_SECO),t_ignition_2a, t_SECO, Y0_2_a, dt);
...
tau_2 = (t_2a - t_ignition_2a)./t_burn_2a;
tau_2 = min(max(tau_2,0),1);
s_2a = 1-(1-tau_2).^2;  % monotone 0->1
theta_2a = theta_SES + (theta_SECO - theta_SES)*s_2a;
...
function dY2adt= burn_2(t,Y2a,stage,m_dot_2a,g,t_ignition_2a,t_burn_2a,theta_SES,theta_SECO)
h=Y2a(1);
v=Y2a(2);
m=Y2a(3);
x=Y2a(4);
...
tau = min(max(tau,0),1);
s = 1-(1-tau).^2;  % monotone 0->1
theta = theta_SES + (theta_SECO - theta_SES)*s;
...
[t1a, Y1a] = rk4_fixed(@(t,Y) burn_1a(t,Y,stage,m_dot_1,g,t_pitchover_optimal,theta_kick_optimal,t_pitch_optimal),0, t_burn_1a, Y0_1a, dt);
...
t_SECO=t_burn_2a_optimal+t_burn_1+t_seperation;

Y0_2_a=[h_SES;v_SES;m_SES;x_SES]; %state at SES

[t_2a, Y2_a] = rk4_fixed(@(t,Y) burn_2(t,Y,stage,m_dot_2a,g,t_ignition_2a,t_burn_2a_optimal,theta_SES,theta_SECO_optimal),t_ignition_2a, t_SECO, Y0_2_a, dt);
...
tau_2 = (t_2a - t_ignition_2a)./t_burn_2a_optimal;
tau_2 = min(max(tau_2,0),1);
s = 1-(1-tau_2).^2;  % monotone 0->1
theta_2a = theta_SES + (theta_SECO_optimal - theta_SES)*s;
...
function dY2adt= burn_2(t,Y2a,stage,m_dot_2a,g,t_ignition_2a,t_burn_2a_optimal,theta_SES,theta_SECO_optimal)
h=Y2a(1);
v=Y2a(2);
m=Y2a(3);
x=Y2a(4);
...
s = 1-(1-tau).^2;  % monotone 0->1
theta = theta_SES + (theta_SECO_optimal - theta_SES)*s;
