...
sol_0=   [10000 ; 10  ; deg2rad(3) ; 10; 7]; %initial guess for m_pld , t_coast , theta_kick , t_pitchover & t_pitch
lowbound=[ 100 ; 2 ; deg2rad(1) ; 5 ; 5]; %best guess for lower bounds
upbound= [ 20000 ; 20 ; deg2rad(10) ; 20 ; 20]; %best guess for upper bounds

objective=@(sol) -sol(1)/1e3;
nonlcon=@(sol) constraints(sol,stage,g,h_orbit,v_orbit,theta_SECO_goal,v_earth,azimuth);
...
function [c,ceq] = constraints(sol,stage,g,h_target,v_target,theta_SECO_goal,v_earth,azimuth)
...
[h_SECO, v_SECO,theta_SECO] = trajectory(m_pld,stage,g,t_coast,t_pitchover, theta_kick,theta_SECO_goal,t_pitch);


v_inertial = v_SECO*cos(theta_SECO) + v_earth;
...
function [h_SECO, v_SECO,theta_SECO] = trajectory( m_pld,stage,g,t_coast,t_pitchover, theta_kick,theta_SECO_goal,t_pitch)
