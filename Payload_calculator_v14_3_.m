...
%% ----Mission Setup----
h_orbit=500e3; %m
const.lat_launch=deg2rad(69.0); 
i_target=deg2rad(97.5); %must be greater than abs(lat_launch)
...
burn_time_max=stage.m_prop_2 /(stage.T_stage2/(stage.isp_2*const.g));
burn_time_guess=300+((burn_time_max-300)/2);

pld_guess=stage.m_wet*0.015;
...
sol_scale = [pld_guess, deg2rad(5), 10, burn_time_guess, deg2rad(6), deg2rad(0.05)];
...
fprintf('av_exitflag %.2f percemt\n', rate_success*100)
...
opts_ode = odeset('RelTol',1e-4,'AbsTol',1e-4, ...
     'Events', @(t,y) stopEvents(t,y,isa,const), ...
     'MaxStep', 2.0);
...
function cache = sim_cached(sol, sol_scale, isa, stage, const, reset)
 ...
 if nargin >= 6 && reset
    ...
    if isempty(last_sol_scaled) || any(sol ~= last_sol_scaled)
...
function [value, isterminal, direction] = stopEvents(t, Y, isa, const)
x = Y(1); z = Y(2); vx = Y(3); vz = Y(4); m = Y(5);

r = hypot(x,z);
h = r - const.r_earth;

% atmosphere model already clamps h<0, but we want to STOP instead
m_min = 1.0;          % kg (or stage.m_dry_2*0.5, etc.)
h_min = 0;            % stop at ground
r_min = const.r_earth * 0.95;

% stop if:
value = [ ...
    h - h_min;        % <=0 => impact
    m - m_min;        % <=0 => mass too small (numerical)
    r - r_min;        % <=0 => inside Earth-ish
    1 - (abs(vx)+abs(vz))/1e6  % <=0 => insane velocity guard (optional)
];
